...
#################### OS Information #####################
#to get the required data in all the fields after field 1 exclusive, hostnamectl command to display OS info with awk command
#-z test command is used to check if the data is available or not by testing the variable and checking if it's empty or not. If empty an error is displayed
...
#################### Old Variables ################################
#to display IP address, hostname command with uppercase "-I" option
#ip=$(hostname -I)

#to display the fqdn information , hostname command with "-f" option is used
#fqdn=$(hostname -f)
...
#An error is printed,if all data items in a section are unavailable,
...
#Created table which is installed with memory components (DIMMs, SODIMMs, etc.) 
...
#Created a table installed with disk drives with each table row having:
#Drive manufacturer
#Drive model
#Drive size in a human friendly format
#Partition number
#Mount point if mounted
#filesystem is mounted if filesystem size exist in a human friendly format
#filesystem is mounted if filesystem free space in human friendly format
